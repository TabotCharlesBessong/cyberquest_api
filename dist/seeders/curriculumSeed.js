...
// Implementation
function q(lessonId, id, typeOrQuestion, maybeQuestionOrCorrect, maybeCorrectOrWrongsOrPayload, maybeExplanationOrDifficulty, maybeDifficultyOrXpReward, maybeXpReward) {
    const QUESTION_TYPES = ["mcq", "matching", "sentence_builder", "investigation"];
    const isNewSignature = typeof typeOrQuestion === "string" && QUESTION_TYPES.includes(typeOrQuestion);
    let type = "mcq";
    let question = "";
    let payload = {};
    let explanation = "";
    ...
    if (isNewSignature) {
        type = typeOrQuestion;
        question = maybeQuestionOrCorrect;
        payload = (maybeCorrectOrWrongsOrPayload || {});
        explanation = maybeExplanationOrDifficulty;
        difficulty = maybeDifficultyOrXpReward || 1;
        xpReward = maybeXpReward || 10;
    }
    else {
        question = typeOrQuestion;
        const correct = maybeQuestionOrCorrect;
        const wrongs = maybeCorrectOrWrongsOrPayload;
        explanation = maybeExplanationOrDifficulty;
        difficulty = maybeDifficultyOrXpReward || 1;
        xpReward = maybeXpReward || 10;
...
function lesson(unitId, id, title, notes, order, ageGroup, difficulty, questions, missionBriefing, voiceOver) {
    return { id: `${unitId}-${id}`, unitId, title, notes, missionBriefing, voiceOver, order, ageGroup, difficulty, questions };
}
function unit(sectionId, id, title, description, icon, order, ageGroup, lessons) {
    return { id: `${sectionId}-${id}`, sectionId, title, description, icon, order, ageGroup, lessons };
}
function section(id, title, description, icon, color, order, ageGroup, units) {
    return { id, title, description, icon, color, order, ageGroup, units };
}
function buildLesson(unitId, ageGroup, def, order) {
    const lessonId = `${unitId}-${def.id}`;
    const questions = def.questions.map((qd) => {
        const type = qd.type || "mcq";
        if (type === "mcq" && qd.correct && qd.wrongs) {
            return q(lessonId, qd.id, qd.question, qd.correct, qd.wrongs, qd.explanation, qd.difficulty ?? def.difficulty, qd.xpReward ?? 10);
        }
        return q(lessonId, qd.id, type, qd.question, {
            options: qd.options,
        ...
        }, qd.explanation, qd.difficulty ?? def.difficulty, qd.xpReward ?? 10);
    });
...
function buildUnit(sectionId, ageGroup, def, order) {
...
function buildSection(def, order) {
...
//# sourceMappingURL=curriculumSeed.js.map