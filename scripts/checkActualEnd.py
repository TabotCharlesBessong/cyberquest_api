...
# Find a-online-safety units array end
start = None
...
# Print last 15 lines of units array
with open("units_end_actual.txt", "w", encoding="utf-8") as f:
    for i in range(max(units_start, units_end - 15), min(units_end + 5, len(lines))):
        f.write(f"{i+1}: {lines[i].rstrip()}\n")

print("Done - check units_end_actual.txt")
