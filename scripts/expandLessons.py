...
for sid, age in existing_sections:
    ...
    for match in reversed(matches):
        ...
        # Find the `{` that starts this lesson - look backward for `{` with only whitespace before the id:
        actual_lesson_start = -1
        for i in range(lesson_start_in_section - 1, -1, -1):
            if section_text[i] == '{':
                # Check if between this `{` and the match start there's only whitespace
        ...
        # Build new lesson text - keep the opening `{` and closing `}`
        # Extract the indentation from the old lesson
        old_lines = old_lesson.split('\n')
        if len(old_lines) > 1:
            indent = old_lines[1][:len(old_lines[1]) - len(old_lines[1].lstrip())]
        else:
            indent = "            "
        ...
        # Remove the trailing ], from questions_text carefully
