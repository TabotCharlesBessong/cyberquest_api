...
# Fix all corrupted section boundaries
content = content.replace('  }as SectionDef,', '  },\n  {')
content = content.replace('  } as SectionDef,', '  },\n  {')
content = content.replace('}as SectionDef,', '},\n  {')
content = content.replace('} as SectionDef,', '},\n  {')

# Now remove the extra sections
EXTRA_SECTIONS = {
    "a-malware-safety",
...
print(f"Found {len(matches)} sections total")
for m in matches:
    print(f"  {m.group(1)}")
...
matches2 = list(section_pattern2.finditer(content))
print(f"\nRemaining sections: {len(matches2)}")
for m in matches2:
    print(f"  {m.group(1)}")
